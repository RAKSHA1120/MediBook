...
function DoctorList() {
  ...
  const locationOptions = useMemo(() => {
    const uniqueLocations = Array.from(new Set(doctorsList.map((d) => d.location).filter(Boolean)));
    return [
  ...
  const filteredDoctors = useMemo(() => {
    return doctorsList.filter((doc) => {
      ...
      const matchesLocation =
        ...
        (doc.location && doc.location.toLowerCase() === selectedLocation.toLowerCase());
